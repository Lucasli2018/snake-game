...
BASE_TPS: 4.25,        // 初始速度：格/秒（难度调低，原 8.5 减半）
MAX_TPS: 10,           // 速度上限（原 20 减半）
TPS_PER_LEVEL: 0.575,  // 每级加速（原 1.15 减半）
SCORE_PER_FOOD: 10,    // 每颗食物得分
SCORE_PER_LEVEL: 100,  // 每多少分升一级（100 分 = 10 颗食物，升级更慢）
```

速度曲线：`tps = min(4.25 + level × 0.575, 10)`，`level = floor(score / 100)`。
即每吃 10 颗食物提速一档，约第 10 档（100 颗食物、1000 分）触顶 10 格/秒。难度已调低。
