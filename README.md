...
- **排行榜**（`localStorage` 持久化，最多 10 条）：点开始屏「🏆 排行榜」按钮进入**独立面板**展示 Top 10，每行显示**玩家名字 + 日期 + 游戏时长 + 模式**（空名字显示「匿名玩家」占位），前三名金银铜奖牌 + 当前玩家行高亮；结束屏显示「第 N / M 名」名次徽章（前三名金银铜配色 + 脉动新纪录）并可改名（默认名 `Ply`，≤10 字符），清榜带 5 秒二次确认倒计时，隐私模式下静默降级
...
- **每日挑战 + 每日榜（J 轮 · 同图公平 + 回流激励）**：开始屏点「📅 每日挑战」进入当日限定挑战——**按日期播种的确定性棋盘**（同一种子 + 同一种子洗牌出的 2 个每日修饰），当天所有玩家同图、公平比拼；点「📅 每日榜」进入独立面板查看今日榜 Top 10（与全局排行榜严格隔离，键 `snake-daily.v1`），每行显示**玩家名字 + 游戏时长 + 日期**。每日修饰共 8 种（`comboBoost` 连击窗口+40% / `greed` 得分×1.5 / `thorns` 每关+3 障碍 / `longSnake` 起步+3 节 / `swift` 基础速度+0.8 / `calm` 基础速度−0.8 / `toxic` 毒区×2 / `noShield` 禁用护盾果），种子由日期 FNV-1a 哈希经 mulberry32 生成；**连续打卡**累计天数（键 `snake-daily-streak.v1`，昨天打卡 +1、其余重置为 1），每连续 1 天给本局星屑 +10% 加成（封顶 +50%），给回流玩家稳定激励
...
- **模式与长线（G 轮 · 把「一局结束」变成「长期旅程」）**：在原来「闯关」的基础上，开始屏新增模式选择与皮肤切换，并补齐成就系统与跨局统计，让每局都有目标与累积意义：
  ...
  - **成就系统（12 项）**：达成瞬间画布内滑入提示卡 + 专属上行音效（`Sfx.achievement`），`localStorage` 持久化（键 `snake-achievements.v1`），重开不丢。判定集中于 `Game.checkAchievements()`，全部 12 项与解锁条件如下。开始屏点「★ 成就」**跳转到独立展示页** `achievements-showcase.html`（零依赖单文件，左上角带「← 返回主页」按钮），按进度条展示已解锁 X/12，逐张列出名称 / 描述 / 解锁条件，并标注该成就解锁的对应皮肤：
    ...
    | 成就 | 名称 | 解锁条件（真实判定） | 类别 |
    | --- | --- | --- | --- |
    | `first` | 初出茅庐 | 累计游戏局数 ≥ 1（`Stats.data.games ≥ 1`） | 进度 |
    | `score100` | 百分达人 | 本局得分 ≥ 100 | 计分 |
    | `score500` | 五百俱乐部 | 本局得分 ≥ 500（并解锁「鎏金」皮肤） | 计分 |
    | `score1000` | 千分传说 | 本局得分 ≥ 1000 | 计分 |
    | `combo5` | 连击新手 | 本局最高连击 ≥ 5 | 连击 |
    | `combo15` | 连击大师 | 本局最高连击 ≥ 15（并解锁「霜蓝」皮肤） | 连击 |
    | `boss1` | 屠龙者 | 本局击破 Boss 数 ≥ 1（并解锁「虚空」皮肤） | 战斗 |
    | `bossSplit` | 弑君者 | 本局击破会分裂的 Boss（`_killedSplitter`） | 战斗 |
    | `clear` | 通关 | 关卡模式胜利（`win && stageMode`，并解锁「赤焰」皮肤） | 进度 |
    | `lucky` | 幸运儿 | 本局吃到一次限时奖励（`_gotBonus`） | 技巧/运气 |
    | `speed` | 风驰电掣 | 当前速度达到上限 `tps() ≥ MAX_TPS`（13 格/秒） | 技巧/运气 |
    | `long` | 长寿蛇 | 本局蛇身长度 ≥ 20 节 | 技巧/运气 |

    > 红线：`first`（完成第一局）依赖累计数据，必须在 `Stats.merge()` 之后才成立 —— `gameOver()` 里先按本局结算一次成就 → 合入统计 → 再结算一次，否则「完成第一局」永不解锁。

  - **皮肤解锁（5 款）**：成就达成后永久解锁对应皮肤（`Skins.isUnlocked` 按成就判定，`currentSkin()` 未解锁自动兜底 `classic`）。配色接入蛇身与蛇头渲染，开始屏点选切换；未解锁带锁标记，点了也不生效。

    | 皮肤 | 名称 | 配色基调 | 解锁方式 |
    | --- | --- | --- | --- |
    | `classic` | 青竹 | 绿 | 默认解锁 |
    | `gold` | 鎏金 | 金 | 成就 `score500`（五百俱乐部） |
    | `void` | 虚空 | 紫 | 成就 `boss1`（屠龙者） |
...
> **红线（UI 不遮挡，全局最高优先级）**：所有遮罩面板（开始 / 暂停 / 结束 / 排行榜 / 好友榜 / 天赋 / 成就 / 每日榜）统一走 `.panel` 布局（`flex-column` + `gap` + `overflow-y:auto`），**禁止绝对定位叠字、负 margin 挤压兄弟元素**；列表类（天赋 / 排行榜 / 好友榜 / 每日榜）走 `.lb-list`（`flex:1 1 auto` + 内部独立滚动），绝不挤压标题 / 按钮 / 信息行。每次新增或改动 UI，**必须目视核对无重叠**——本项目已因天赋面板文字叠压、GAME OVER 每日信息行压住按钮两处遮挡返工，列为硬约束。
...
把「一局结束」变成「长期养成」：每局结算给 ✨星屑，攒够后在开始屏「🌟 天赋」面板解锁**永久**天赋，跨所有对局生效，与单局内的升级卡（E 轮）叠加。

**星屑来源**（结算时一次性计入）：

```
earned = floor(score / 20) + comboBest×2 + runBossKills×5 + (通关 ? 30 : 0) + (新纪录 ? 15 : 0)
```

**6 种天赋**（星级越高越强，花费随等级递增；等级 0 时与旧版行为完全一致）：

| 天赋 | 图标 | 效果 | 最大等级 | 各级花费 |
| --- | --- | --- | --- | --- |
| 强身 | ♥ | 初始生命 +1 | 2 | 20 / 45 |
| 贪婪之魂 | ★ | 全局得分 +5% | 3 | 15 / 30 / 55 |
| 连击感知 | ⚡ | 连击窗口 +1 秒 | 3 | 12 / 25 / 45 |
| 幸运星 | 🍀 | 特殊食物刷新间隔 −20% | 3 | 15 / 30 / 55 |
| 稳健 | ❄ | 基础速度 −0.8 格/秒 | 2 | 20 / 45 |
| 长蛇 | 🐍 | 起步蛇身 +1 节 | 2 | 18 / 40 |

**接入位置**：`Game.reset()` 时读取 `Meta` 天赋，叠加到 `lives / maxLives / comboWindow / scoreMul / calmLevel / specialInterval / 起步长度`；星屑在 `Game.gameOver()` 末尾累加进 `Meta`。数据持久化在 `localStorage` 键 `snake-meta.v1`（结构 `{ stardust, talents:{id:rank} }`），脏数据 / 隐私模式（写入被拒绝）一律 `try-catch` 兜底为默认，绝不抛异常。
...
```js
{ name: string(≤10字符), score: number, date: ISO date string, duration: number(秒) }
```

- 名次按分数降序、同分按时间倒序（新的在前）
- **同名同分视为同一条记录**：再次提交会替换旧记录并保留最新时间
- 结束一局后默认以 `Ply` 入榜，可在结束屏点击「改名」改成 ≤10 字符、回车或点「保存」确认（改名即换名，成绩与时间不变）
...
> 全套 `cd tests && node --test` 共 **335** 条，全绿（含 8 条 J 轮每日挑战测试 + 1 条排行榜显示回归测试 `10.22`）。
