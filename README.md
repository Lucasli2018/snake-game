...
BASE_TPS: 8.5,         // 初始速度：格/秒
MAX_TPS: 20,           // 速度上限
TPS_PER_LEVEL: 1.15,   // 每级加速
SCORE_PER_FOOD: 10,    // 每颗食物得分
SCORE_PER_LEVEL: 50,   // 每多少分升一级（50 分 = 5 颗食物）
```

速度曲线：`tps = min(8.5 + level × 1.15, 20)`，`level = floor(score / 50)`。
即每吃 5 颗食物提速一档，约第 10 档（50 颗食物、500 分）触顶 20 格/秒。
