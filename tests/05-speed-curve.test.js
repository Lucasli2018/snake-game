 ...
 * 05 - 速度曲线与升级阈值
 * 覆盖：
 *   - tps = min(8.5 + level*1.15, 20) 单调递增、有上限、有下限
 *   - tickInterval 单调递减
 *   - level = floor(score / 50)，每 50 分（5 颗食物）升一级
 *   - 连续吃 10 颗食物时每 5 颗升一级
...
test('05.1 Config 常量与设计一致', () => {
  ...
  assert.strictEqual(c.BASE_TPS, 8.5);
  assert.strictEqual(c.MAX_TPS, 20);
  assert.strictEqual(c.TPS_PER_LEVEL, 1.15);
  assert.strictEqual(c.SCORE_PER_FOOD, 10);
  assert.strictEqual(c.SCORE_PER_LEVEL, 50);
  assert.strictEqual(c.SCORE_PER_LEVEL / c.SCORE_PER_FOOD, 5, '每 5 颗食物升一级');
});

test('05.2 tps 随 level 单调不减，且恒在 [8.5, 20] 区间内', () => {
  const h = H.createHarness();
  const g = h.Game;
  let prev = -Infinity;
  for (let level = 0; level <= 200; level++) {
    ...
    assert.ok(t <= h.Config.MAX_TPS + 1e-9, `level ${level}：tps ${t} 超过上限 20`);
    prev = t;
  }
});

test('05.3 tps 严格等于 min(8.5 + level*1.15, 20)', () => {
  const h = H.createHarness();
  const g = h.Game;
  for (let level = 0; level <= 60; level++) {
    g.level = level;
    const expected = Math.min(20, 8.5 + level * 1.15);
...
test('05.4 速度上限在 level 10 触顶，之后不再增长', () => {
  ...
  g.level = 9;
  const t9 = g.tps();
  assert.ok(t9 < 20, `level 9 尚未触顶（${t9}）`);

  g.level = 10;
  assert.ok(Math.abs(g.tps() - 20) < 1e-9, `level 10 应恰好触顶 20，实际 ${g.tps()}`);

  for (const lv of [11, 20, 50, 100, 999]) {
    g.level = lv;
    assert.ok(Math.abs(g.tps() - 20) < 1e-9, `level ${lv} 时 tps 应保持 20，实际 ${g.tps()}`);
  }
});
...
test('05.5 tickInterval = 1000/tps，随等级单调递减', () => {
  ...
  // 最快 20 格/秒 -> 50ms 一步
  g.level = 999;
  assert.strictEqual(g.tickInterval(), 50);
});

/* ================================================================== *
 * 2. 升级阈值与得分逻辑
 * ================================================================== */
test('05.6 level 恒等于 floor(score / 50)', () => {
  const h = H.createHarness();
  const g = h.Game;
  ...
  for (const s of scores) {
    ...
    assert.strictEqual(gg.level, Math.floor(newScore / h.Config.SCORE_PER_LEVEL),
      `score ${newScore} 对应的 level 应为 ${Math.floor(newScore / 50)}，实际 ${gg.level}`);
  }
});

test('05.7 连续吃 10 颗：每 5 颗升一级，得分每次 +10', () => {
  const h = H.createHarness();
  const g = fresh(h);
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    assert.strictEqual(g.level, Math.floor((i * 10) / 50),
      `第 ${i} 颗后等级应为 ${Math.floor(i * 10 / 50)}，实际 ${g.level}`);
    assert.strictEqual(g.snake.length, h.Config.START_LEN + i,
      `第 ${i} 颗后蛇长应为 ${h.Config.START_LEN + i}，实际 ${g.snake.length}`);
  }

  // 断点检查：第 5 颗（50 分）升到 1 级，第 10 颗（100 分）升到 2 级
  assert.strictEqual(trace[4].level, 1, '第 5 颗食物（50 分）应升到 Lv.2(level=1)');
  assert.strictEqual(trace[9].level, 2, '第 10 颗食物（100 分）应升到 Lv.3(level=2)');
...
test('05.9 达到 level 10 约需 500 分（50 颗食物），与 README 描述一致', () => {
  const h = H.createHarness();
  const g = fresh(h);
  g.score = 500 - h.Config.SCORE_PER_FOOD;   // 吃下这颗后正好 500
  feedOnce(h);
  assert.strictEqual(g.score, 500);
  assert.strictEqual(g.level, 10);
  assert.ok(Math.abs(g.tps() - 20) < 1e-9, '500 分时速度应已触顶 20 格/秒');
});
...
test('05.10 HUD 显示的等级、速度、进度条与内部状态一致', () => {
  ...
  for (const level of [0, 1, 5, 10, 20]) {
    ...
    assert.ok(pct >= 4 && pct <= 100, `进度条宽度 ${width} 越界`);
